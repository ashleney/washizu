...
impl Invisible {
    pub fn new(game: &[Event], trust_seed: bool) -> Vec<Self> {
        ...
        for event in game {
            match event {
                Event::StartGame { seed: Some(game_seed), .. } if trust_seed => {
                    seed = Some(*game_seed);
                }
                Event::StartKyoku { bakaze, kyoku, honba, dora_marker, tehais, .. } => {
            ...
            match event {
                ...
                Event::Hora { ura_markers: Some(ura), .. } if !ura_is_recorded => {
                    for &tile in ura {
                ...
                Event::EndKyoku => {
                    let mut filler: Vec<_> = unknown_tiles
                        ...
                        .flat_map(|(tid, count)| iter::repeat_n(
                            must_tile!(tid),
                            count as usize,
                        ))
                        .collect();
...
pub const fn new_unknown_tiles() -> [u8; 37] {
    let mut ret = [4; 37];
    ret[tuz!(5m)] = 3;
    ret[tuz!(5p)] = 3;
    ret[tuz!(5s)] = 3;
