...
impl Grp {
    #[staticmethod]
    pub fn load_log(raw_log: &str) -> Result<Self> {
        let events = raw_log
            .lines()
            .map(json::from_str)
    ...
    pub fn load_gz_log_files_py(gzip_filenames: Vec<PyBackedStr>) -> Result<Vec<Self>> {
    ...
    pub fn load_events(events: &[Event]) -> Result<Self> {
        ...
        for ev in events.iter().rev() {
            match *ev {
                ...
                Event::StartKyoku { bakaze, kyoku, honba, kyotaku, scores, .. } => {
                    if rank_by_player_opt.is_none() {
                        final_scores = scores;
        ...
        let rank_by_player = rank_by_player_opt
            .context("invalid log: no Hora or Ryukyoku after a StartKyoku")?;
        let shape = (game_info.len(), GRP_SIZE);
        let feature = Array::from_iter(game_info.into_iter().flatten())
