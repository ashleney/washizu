...
pub static JIHAI_TABLE: LazyLock<Vec<[u8; 10]>> = LazyLock::new(|| {
    read_table(include_bytes!("data/shanten_jihai.bin.gz"), JIHAI_TABLE_SIZE)
});
pub static SUHAI_TABLE: LazyLock<Vec<[u8; 10]>> = LazyLock::new(|| {
    read_table(include_bytes!("data/shanten_suhai.bin.gz"), SUHAI_TABLE_SIZE)
});
pub fn read_table(gzipped: &[u8], length: usize) -> Vec<[u8; 10]> {
...
pub fn add_suhai(lhs: &mut [u8; 10], index: usize, m: usize) {
...
pub fn add_jihai(lhs: &mut [u8; 10], index: usize, m: usize) {
...
pub fn sum_tiles(tiles: &[u8]) -> usize {
...
pub fn calc_all(tiles: &[u8; 34], len_div3: u8) -> i8 {
    ...
    if shanten > 0 { shanten.min(calc_kokushi(tiles)) } else { shanten }
}
#[cfg(test)]
pub mod test {
    use super::*;
