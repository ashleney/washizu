...
pub struct Candidate {
    ...
    pub yaku_names: ArrayVec<[Vec<(String, f32)>; MAX_TSUMOS_LEFT]>,
}
#[derive(Default)]
pub struct RawCandidate<'a> {
    ...
    pub required_tiles: ArrayVec<[RequiredTile; 34]>,
    pub shanten_down: bool,
    pub yaku_names: &'a [AHashMap<String, f32>],
}
#[derive(Clone, Copy)]
...
impl From<RawCandidate<'_>> for Candidate {
    fn from(
        ...
        let yaku_names = yaku_names
            .iter()
            .map(|x| {
                let mut x: Vec<(String, f32)> = x.iter().map(|(y, v)| (y.clone(), v.clamp(0., 1.0))).collect();
...
impl Candidate {
    ...
    pub fn calibrate(mut self, real_max_tsumo: usize) -> Self {
