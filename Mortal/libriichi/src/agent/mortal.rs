...
pub struct MortalBatchAgent {
    ...
    pub is_oracle: bool,
    pub version: u32,
    pub enable_quick_eval: bool,
    pub enable_rule_based_agari_guard: bool,
    pub name: String,
    pub player_ids: Vec<u8>,
    pub actions: Vec<usize>,
    pub q_values: Vec<[f32; ACTION_SPACE]>,
    pub masks_recv: Vec<[bool; ACTION_SPACE]>,
    pub is_greedy: Vec<bool>,
    pub last_eval_elapsed: Duration,
    pub last_batch_size: usize,
    pub evaluated: bool,
    pub quick_eval_reactions: Vec<Option<Event>>,
    pub wg: WaitGroup,
    pub sync_fields: Arc<Mutex<SyncFields>>,
}
pub struct SyncFields {
    pub states: Vec<Array2<f32>>,
    pub invisible_states: Vec<Array2<f32>>,
    pub masks: Vec<Array1<bool>>,
    pub action_idxs: Vec<usize>,
    pub kan_action_idxs: Vec<Option<usize>>,
}
impl MortalBatchAgent {
    pub fn new(engine: PyObject, player_ids: &[u8]) -> Result<Self> {
        ...
        let (
            ...
            let obj = engine.bind_borrowed(py);
                ...
                obj.getattr("react_batch") ?.is_callable(), "missing method react_batch",
            );
            ...
            let enable_rule_based_agari_guard = obj
        ...
        let sync_fields = Arc::new(
            Mutex::new(SyncFields {
                states: vec![],
    ...
    pub fn evaluate(&mut self) -> Result<()> {
        ...
        (self.actions, self.q_values, self.masks_recv, self.is_greedy) = Python::with_gil(|
            py|
        {
            ...
            let masks: Vec<_> = sync_fields
                ...
                .collect();
            let invisible_states: Option<Vec<_>> = self
                .is_oracle
                .then(|| {
                    sync_fields
                        .invisible_states
    ...
    pub fn gen_meta(&self, state: &PlayerState, action_idx: usize) -> Metadata {
...
impl BatchAgent for MortalBatchAgent {
    ...
    fn set_scene(
        ...
        if self.enable_quick_eval && cans.can_discard && !cans.can_riichi
            && !cans.can_tsumo_agari && !cans.can_ankan && !cans.can_kakan
            && !cans.can_ryukyoku
        {
    ...
    fn get_reaction(
        ...
        let action = if self.enable_rule_based_agari_guard && orig_action == 43
        ...
        let event = match action {
            0..=36 => {
                    ...
                    cans.can_discard, "failed discard check: {}", state.brief_info()
                );
            ...
            37 => {
                ensure!(cans.can_riichi, "failed riichi check: {}", state.brief_info());
                Event::Reach { actor }
            }
            38 => {
                ensure!(
                    cans.can_chi_low, "failed chi low check: {}", state.brief_info()
                );
                let pai = state
                    .last_kawa_tile()
            ...
            39 => {
                    ...
                    cans.can_chi_mid, "failed chi mid check: {}", state.brief_info()
                );
                let pai = state
                    .last_kawa_tile()
                    .context("invalid state: no last kawa tile")?;
            ...
            40 => {
                    ...
                    cans.can_chi_high, "failed chi high check: {}", state.brief_info()
                );
                let pai = state
                    .last_kawa_tile()
                    .context("invalid state: no last kawa tile")?;
            ...
            42 => {
                    ...
                    "failed kan check: {}", state.brief_info()
                );
                ...
                let tile = if let Some(kan_idx) = kan_select_idx {
                    let tile = must_tile!(self.actions[kan_idx]);
                        ...
                        ankan_candidates.contains(& tile) || kakan_candidates.contains(&
                        tile), "kan choice not in kan candidates: {}", state.brief_info()
                    );
                    tile
                } else if cans.can_daiminkan {
                    state.last_kawa_tile().context("invalid state: no last kawa tile")?
            ...
            43 => {
                ensure!(cans.can_agari(), "failed hora check: {}", state.brief_info(),);
                Event::Hora {
                    actor,
                    target: cans.target_actor,
                    deltas: None,
            ...
            44 => {
                    ...
                    cans.can_ryukyoku, "failed ryukyoku check: {}", state.brief_info()
                );
        ...
        meta.kan_select = kan_select_idx
            .map(|kan_idx| Box::new(self.gen_meta(state, kan_idx)));
        Ok(EventExt {
            event,
            meta: Some(meta),
        })
