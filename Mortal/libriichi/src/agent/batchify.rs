...
{
    pub inner: Vec<A>,
    pub last_actions: Vec<Option<EventExt>>,
}
    ...
    A: Agent,
{
        ...
        F: FnMut(u8) -> Result<A>,
    {
        ...
        let inner = player_ids.iter().copied().map(new_fn).collect::<Result<_>>()?;
        Ok(Self {
            inner,
    ...
    A: Agent,
{
    ...
    fn set_scene(
        ...
        self.last_actions[index] = Some(
            self.inner[index].react(log, state, invisible_features)?,
        );
        Ok(())
    }
    #[inline]
    fn get_reaction(
        ...
        self.last_actions[index].take().context("`get_reaction` without `set_scene`")
    }
    #[inline]
