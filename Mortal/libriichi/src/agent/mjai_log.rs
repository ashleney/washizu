...
pub struct MjaiLogBatchAgent {
    ...
    pub reactions: Vec<Option<EventExt>>,
    pub reactions_idxs: Vec<usize>,
    pub evaluated: bool,
}
#[pyclass]
#[derive(Default)]
    ...
    pub fn new(engine: PyObject, player_ids: &[u8]) -> Result<Self> {
        ...
        let name = Python::with_gil(|py| {
            ...
            for method in ["react_batch", "start_game", "end_kyoku", "end_game"] {
                ensure!(obj.getattr(method) ?.is_callable(), "missing method {method}",);
            }
...
impl BatchAgent for MjaiLogBatchAgent {
    ...
    fn get_reaction(
        ...
        let event = self.reactions[reactions_idx].take().context("take after take")?;
        Ok(event)
    }
