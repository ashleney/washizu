...
pub struct AkochanAgent {
    ...
    pub event_idx: usize,
    pub naki_tx: Option<Event>,
}
impl AkochanAgent {
    pub fn new(player_id: u8) -> Result<Self> {
        ensure!(matches!(player_id, 0..= 3));
        let akochan_dir = env::var_os("AKOCHAN_DIR")
            .unwrap_or_else(|| OsString::from("akochan"));
        let akochan_exe = [&akochan_dir, OsStr::new("system.exe")]
            .iter()
            .collect::<PathBuf>();
        let akochan_tactics = env::var_os("AKOCHAN_TACTICS")
            .unwrap_or_else(|| OsString::from("tactics.json"));
        ...
        let stdin = child.stdin.take().context("failed to get stdin of akochan")?;
        let stdout = child.stdout.take().context("failed to get stdout of akochan")?;
...
impl Agent for AkochanAgent {
    ...
    fn react(
        ...
        if let Some(dahai) = self.naki_tx.take() {
            ...
            match last.event {
                ...
                | Event::Reach { actor, .. } if actor == self.player_id => {
    ...
    fn start_game(&mut self) -> Result<()> {
        let start_game = json::json!(
            { "type" : "start_game", "kyoku_first" : 0, "aka_flag" : true, }
        );
