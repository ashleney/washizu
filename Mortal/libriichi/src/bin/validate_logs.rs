...
pub fn process_path(path: &Path) -> Result<()> {
    let raw_log = if path.extension().is_some_and(|s| s.eq_ignore_ascii_case("gz")) {
    ...
    for (idx, ev) in events.iter().enumerate() {
        ...
        match ev {
            Event::Dahai { actor, pai, .. } => {
                ...
                let discard_candidates = states[*actor as usize]
            ...
            Event::Chi { actor, consumed, pai, target } => {
                ensure!(
                    (target + 1) % 4 == * actor,
                ...
                match ChiType::new(*consumed, *pai) {
                    ChiType::Low => {
                            ...
                            line, states[* actor as usize].brief_info(),
                        );
                    }
                    ChiType::Mid => {
                        ensure!(
            ...
            Event::Hora { actor, target, ura_markers, deltas } => {
                ...
                let ura = ura_markers.as_ref().context("missing field `ura_markers`")?;
