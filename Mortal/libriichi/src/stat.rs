...
Nagashi mangan (rate) {} ({:.9})"#,
            self.game, self.round, self.oya, self.rank_1, self.rank_1_rate(), self
            .rank_2, self.rank_2_rate(), self.rank_3, self.rank_3_rate(), self.rank_4,
            self.rank_4_rate(), self.tobi, self.tobi_rate(), self.avg_rank(), self
            .total_pt([90, 45, 0, - 135]), self.avg_pt([90, 45, 0, - 135]), self.point,
            self.avg_point_per_game(), self.avg_point_per_round(), self.agari_rate(),
            self.houjuu_rate(), self.fuuro_rate(), self.riichi_rate(), self
            .ryukyoku_rate(), self.avg_point_per_agari(), self.avg_point_per_oya_agari(),
            self.avg_point_per_ko_agari(), self.avg_point_per_riichi_agari(), self
            .avg_point_per_fuuro_agari(), self.avg_point_per_dama_agari(), self
            .avg_point_per_ryukyoku(), self.avg_agari_jun(), self.avg_riichi_agari_jun(),
            self.avg_fuuro_agari_jun(), self.avg_dama_agari_jun(), self.avg_houjuu_jun(),
            self.avg_point_per_houjuu(), self.avg_point_per_houjuu_to_oya(), self
            .avg_point_per_houjuu_to_ko(), self.chasing_riichi_rate(), self
            .riichi_chased_rate(), self.agari_rate_after_riichi(), self
            .houjuu_rate_after_riichi(), self.avg_riichi_jun(), self.avg_riichi_point(),
            self.avg_fuuro_num(), self.agari_rate_after_fuuro(), self
            .houjuu_rate_after_fuuro(), self.avg_fuuro_point(), self.agari_rate_as_oya(),
            self.agari_as_oya_rate(), self.houjuu_to_oya_rate(), self.yakuman, self
            .yakuman_rate(), self.nagashi_mangan, self.nagashi_mangan_rate(),
        )
    }
}
impl Stat {
    ...
    pub fn from_game(events: &[Event], player_id: u8) -> Self {
        ...
        events
            ...
            .for_each(|ev| match *ev {
                ...
                Event::Chi { actor, .. }
                | Event::Pon { actor, .. }
                | Event::Daiminkan { actor, .. } if actor == player_id => {
                    fuuro_num += 1;
                }
                Event::Reach { actor } => {
                    if actor == player_id {
    ...
    pub fn from_dir(
        ...
        let bar = if disable_progress_bar {
            ...
            ProgressBar::new_spinner()
                .with_style(
                    ProgressStyle::with_template(TEMPLATE)?.tick_chars(".oO°Oo*"),
                )
        };
