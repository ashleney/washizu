...
impl BoardState {
    ...
    #[inline]
    pub fn add_log(&mut self, ev: EventExt) {
        self.log.push(ev);
    }
    #[inline]
    pub fn add_log_no_meta(&mut self, ev: Event) {
    ...
    pub fn haipai(&mut self) -> Result<()> {
        ...
        let tile = self.board.yama.pop().context("invalid yama: empty at init")?;
        self.tiles_left -= 1;
    ...
    pub fn exhaustive_ryukyoku(&mut self) {
        let mut deltas = [0; 4];
    ...
    pub const fn update_nagashi_mangan_and_four_wind(&mut self, ev: &Event) {
        match *ev {
    ...
    pub fn check_four_wind(&mut self, pai: Tile) -> Result<bool> {
    ...
    pub fn check_riichi_accepted(&mut self) {
    ...
    pub fn add_new_dora(&mut self) -> Result<()> {
        let dora = self
            .board
            .dora_indicators
            .pop()
    ...
    pub fn handle_hora(
        ...
        let mut honba_left = self.board.honba as i32;
        let mut kyotaku_point = self.board.kyotaku as i32 * 1000;
        self.board.kyotaku = 0;
        let ura_indicators = self
            .board
            .ura_indicators[..5 - self.board.dora_indicators.len()]
            .to_vec();
            ...
            points
                ...
                .for_each(|(actor, point)| {
                    ...
                    if let Some(pao_target) = self.paos[actor] {
                        ...
                        deltas[single_target as usize] -= point.ron / 2;
                    } else {
        ...
        deltas[single_actor as usize] = point.tsumo_total(single_actor == self.oya)
            + kyotaku_point + honba_left * 300;
        vec_add_assign(&mut self.kyoku_deltas, &deltas);
        let ura_markers = self
            .player_states[single_actor as usize]
            .self_riichi_accepted()
    ...
    pub fn update_paos(&mut self, ev: &Event) {
        match *ev {
            Event::Pon { target, actor, pai, .. }
            | Event::Daiminkan { target, actor, pai, .. } if pai.is_jihai() => {
                let mut jihais = 0_u8;
    ...
    #[inline]
    pub fn abortive_ryukyoku(&mut self) {
        let ryukyoku = Event::Ryukyoku {
            deltas: Some([0; 4]),
        };
        self.add_log_no_meta(ryukyoku);
    ...
    pub fn step(&mut self, reactions: &[EventExt; 4]) -> Result<Poll> {
        ...
        match ev.event {
            Event::None => {
                ...
                let tile = if self.deal_from_rinshan.take().is_some() {
                    ...
                    self.board
                        ...
                        .with_context(|| {
                            format!(
                                "tiles left > 0 ({}) but yama is empty", self.tiles_left
                            )
                        })?
                };
                self.tiles_left -= 1;
            ...
            Event::Dahai { actor, pai, .. } => {
                ...
                if self.kans == 4
                    && self.player_states.iter().all(|s| s.kans_count() < 4)
                {
