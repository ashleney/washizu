...
pub struct Game {
    ...
    pub seed: (u64, u64),
    pub indexes: [Index; 4],
    pub oracle_obs_versions: [Option<u32>; 4],
    pub invisible_state_cache: [Option<Array2<f32>>; 4],
    pub last_reactions: [EventExt; 4],
...
impl Game {
    ...
    pub fn poll(&mut self, agents: &mut [Box<dyn BatchAgent>]) -> Result<()> {
        if self.ended {
            return Ok(());
        }
        if !self.kyoku_started {
            if self.kyoku >= self.length + 4
                || self.kyoku >= self.length && !self.in_renchan
        ...
        match poll {
            Poll::InGame => {
                ...
                for (player_id, state) in ctx.player_states.iter().enumerate() {
                    ...
                    let invisible_state = self
                        .oracle_obs_versions[player_id]
                    ...
                    agents[idx.agent_idx]
                        .set_scene(idx.player_id_idx, ctx.log, state, invisible_state)?;
                }
            }
            Poll::End => {
    ...
    pub fn commit(
        ...
        if self.ended {
            if self.kyotaku > 0 {
                *self.scores.iter_mut().min_by_key(|s| -**s).unwrap()
        ...
        for (player_id, state) in ctx.player_states.iter().enumerate() {
            ...
            let idx = self.indexes[player_id];
            self.last_reactions[player_id] = agents[idx.agent_idx]
                .get_reaction(idx.player_id_idx, ctx.log, state, invisible_state)?;
        }
        Ok(None)
    }
}
impl BatchGame {
    ...
    pub fn run(
        ...
        seeds: &[(u64, u64)],
            ...
            "expected `indexes.len() == seeds.len()`, got {} and {}", indexes.len(),
            seeds.len(),
        );
        ...
        while !games.is_empty() {
            ...
            bar.set_message(
                format!(
                    ...
                    cycles as f64 / secs, actions as f64 / secs,
                ),
            );
        }
        bar.abandon();
...
pub mod test {
    ...
    pub fn tsumogiri() {
        ...
        g.run(&mut agents, indexes, &[(1009, 0), (1021, 0)]).unwrap();
    }
}
