...
impl TwoVsTwo {
    ...
    pub const fn new(disable_progress_bar: bool, log_dir: Option<String>) -> Self {
        Self {
    ...
    pub fn ako_vs_py(
        ...
        py.allow_threads(move || {
            self.run_batch(
                |player_ids| {
                    AkochanAgent::new_batched(player_ids).map(|a| Box::new(a) as _)
                },
...
impl TwoVsTwo {
        ...
        M: FnOnce(&[u8]) -> Result<Box<dyn BatchAgent>>,
    {
        ...
        log::info!(
            "seed: [{}, {}) w/ {:#x}, start {} sets, {} hanchans", seed_start.0,
            seed_start.0 + seed_count, seed_start.1, seed_count, seed_count * 2,
        );
        let seeds: Vec<_> = (seed_start.0..seed_start.0 + seed_count)
            .flat_map(|seed| iter::repeat_n((seed, seed_start.1), 2))
            .collect();
        let challenger_player_ids_per_seed = [0, 2, 1, 3];
        let challenger_player_ids: Vec<_> = challenger_player_ids_per_seed
            ...
            .collect();
        let champion_player_ids_per_seed = [1, 3, 0, 2];
            ...
            results
                ...
                .try_for_each(|(i, game_result)| {
                    ...
                    let filename: PathBuf = [
                        dir,
                        &format!("{seed}_{key}_{split_name}.json.gz"),
                    ]
                        .iter()
                        .collect();
        ...
        M: FnOnce(&[u8]) -> Result<Box<dyn BatchAgent>>,
    {
        ...
        log::info!(
            "seed: {} w/ {:#x}, split: {}, start 1 hanchan", seed.0, seed.1, split
        );
