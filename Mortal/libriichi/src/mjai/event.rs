...
pub enum Event {
    ...
    Tsumo { #[serde_as(deserialize_as = "TryFromInto<Actor>")] actor: u8, pai: Tile },
    Dahai {
        #[serde_as(deserialize_as = "TryFromInto<Actor>")]
        actor: u8,
        pai: Tile,
    ...
    Dora { dora_marker: Tile },
    Reach { #[serde_as(deserialize_as = "TryFromInto<Actor>")] actor: u8 },
    ReachAccepted { #[serde_as(deserialize_as = "TryFromInto<Actor>")] actor: u8 },
    Hora {
        #[serde_as(deserialize_as = "TryFromInto<Actor>")]
        actor: u8,
        #[serde_as(deserialize_as = "TryFromInto<Actor>")]
...
impl Event {
    ...
    pub const fn is_in_game_announce(&self) -> bool {
        matches!(
            self, Self::ReachAccepted { .. } | Self::Dora { .. } | Self::Hora { .. }
        )
    }
    pub fn augment(&mut self) {
        pub const fn swap_tile(t: &mut Tile) {
            *t = t.augment();
        }
        match self {
            Self::StartKyoku { bakaze, dora_marker, tehais, .. } => {
                swap_tile(bakaze);
...
pub mod test {
    ...
    #[test]
    pub fn bound_check() {
        let value = json!({ "type" : "reach", "actor" : 4, });
        json::from_value::<Event>(value).unwrap_err();
        let value = json!({ "type" : "hora", "actor" : 0, "target" : 5, });
        json::from_value::<Event>(value).unwrap_err();
        let value = json!(
            { "type" : "start_kyoku", "bakaze" : "E", "dora_marker" : "5s", "kyoku" : 1,
            "honba" : 0, "kyotaku" : 0, "oya" : 0, "scores" : [25000, 25000, 25000,
            25000], "tehais" : [["N", "3p", "W", "W", "7m", "N", "S", "C", "7m", "P",
            "8p", "2m", "5m"], ["7p", "1p", "2m", "3m", "4m", "C", "7s", "7s", "9s",
            "9p", "1m", "C", "1s"], ["3s", "E", "5m", "P", "5m", "F", "7p", "6m", "5s",
            "9p", "1s", "S", "N"], ["2p", "4s", "4p", "E", "5p", "F", "3p", "1s", "8p",
            "6s", "8s", "7s", "5p"],], }
        );
