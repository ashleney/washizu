...
use std::mem;
impl PlayerState {
    pub fn test_update(&mut self, event: &Event) -> ActionCandidate {
        let cans = self.update(event).unwrap();
        self.validate();
        cans
    }
    pub fn test_update_json(&mut self, mjai_json: &str) -> ActionCandidate {
        let cans = self.update_json(mjai_json).unwrap();
        self.validate();
        cans
    }
    pub fn from_log(player_id: u8, log: &str) -> Self {
    ...
    pub fn num_doras_in_hand(&self) -> u8 {
        self.tehai
            .iter()
    ...
    pub fn validate(&self) {
        assert_eq!(
            self.real_time_shanten(), shanten::calc_all(& self.tehai, self
            .tehai_len_div3),
        );
        assert_eq!(
...
pub fn can_chi() {
    ...
    assert!(
        matches!(ps.last_cans, ActionCandidate { can_chi_high : false, can_chi_mid :
        false, can_chi_low : false, .. },)
    );
    ...
    assert!(
        matches!(ps.last_cans, ActionCandidate { can_chi_high : true, can_chi_mid : true,
        can_chi_low : false, .. },)
    );
    ...
    assert!(
        matches!(ps.last_cans, ActionCandidate { can_chi_high : true, can_chi_mid :
        false, can_chi_low : false, .. },)
    );
...
pub fn dora_count_after_kan() {
    ...
    ps.test_update(&Event::Dora { dora_marker: t!(9s) });
    ps.test_update(
        &Event::Tsumo {
            actor: 0,
            pai: t!(5pr),
    ...
    ps.test_update(&Event::Dora { dora_marker: t!(4p) });
    assert_eq!(ps.doras_owned[0], 8);
}
#[test]
pub fn rule_based_agari_all_last_minogashi() {
    let log = r#"
...
#[test]
pub fn discard_candidates_with_unconditional_tenpai() {
    let log = r#"
...
pub fn double_chankan_ron() {
    ...
    let cans = ps_kakan
        .test_update_json(
            r#"{"type":"kakan","actor":3,"pai":"2m","consumed":["2m","2m","2m"]}"#,
        );
    assert!(cans.can_ron_agari);
    assert_eq!(ps_kakan.agari_points(true, & []).unwrap().ron, 1000);
    let cans = ps
        .test_update_json(r#"{"type":"dahai","actor":3,"pai":"2m","tsumogiri":true}"#);
    assert!(! cans.can_ron_agari);
}
#[test]
pub fn chi_at_0_shanten() {
    let log = r#"
    ...
    ps.test_update_json(
        r#"{"type":"chi","actor":0,"target":3,"consumed":["4s","5s"],"pai":"6s"}"#,
    );
    assert_eq!(ps.shanten, 0);
