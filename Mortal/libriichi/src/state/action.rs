...
impl ActionCandidate {
    ...
    pub const fn can_act(&self) -> bool {
        self.can_discard || self.can_chi() || self.can_pon || self.can_kan()
            || self.can_riichi || self.can_agari() || self.can_ryukyoku
    }
...
impl PlayerState {
    ...
    pub fn validate_reaction(&self, action: &Event) -> Result<()> {
        ...
        if let Some(actor) = action.actor() {
                ...
                actor == self.player_id, "actor is {actor}, not self ({})", self
                .player_id,
            );
        } else {
        ...
        match *action {
            ...
            Event::Kakan { pai, .. } => {
                    ...
                    self.kakan_candidates.contains(& pai.deaka()), "cannot kakan {pai}",
                );
                self.ensure_tiles_in_hand(&[pai])?;
            }
    ...
    pub fn ensure_tiles_in_hand(&self, tiles: &[Tile]) -> Result<()> {
        for &tile in tiles {
