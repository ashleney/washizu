...
const MAX_NUM_TURNS: usize = 17;
pub struct ObsEncoderContext<'a> {
    pub state: &'a PlayerState,
    pub arr: Simple2DArray<34, f32>,
...
pub struct IntegerEncoder {
    ...
    pub one_hot: bool,
    pub rescale: bool,
    pub rbf_intervals: Option<NonZeroUsize>,
}
impl IntegerEncoder {
    pub const fn new(n: usize, cap: usize) -> Self {
        Self {
            n,
            cap,
            one_hot: false,
    ...
    pub const fn one_hot(mut self, v: bool) -> Self {
        self.one_hot = v;
        self
    }
    pub const fn rescale(mut self, v: bool) -> Self {
        self.rescale = v;
        self
    }
    pub const fn rbf_intervals(mut self, v: usize) -> Self {
        self.rbf_intervals = NonZeroUsize::new(v);
        self
    }
    pub fn encode(self, ctx: &mut ObsEncoderContext<'_>) {
        ...
        match ctx.version {
            ...
            2 | 3 => {
                debug_assert!(
                    self.one_hot || self.rescale || self.rbf_intervals.is_some()
                );
                if self.one_hot {
...
impl<'a> ObsEncoderContext<'a> {
    pub fn new(state: &'a PlayerState, version: u32, at_kan_select: bool) -> Self {
    ...
    pub fn encode_obs(mut self) -> (Array2<f32>, Array1<bool>) {
        ...
        if matches!(self.version, 2 | 3 | 4) {
            ...
            IntegerEncoder::new(n as usize, 7).rescale(true).encode(&mut self);
        }
    ...
    pub fn encode_ev(&mut self, value: f32) {
    ...
    pub fn encode_sp_table(
        ...
        let Some(first) = candidates
            .first()
            .filter(|c| c.tenpai_probs.first().is_some_and(|&p| p > 0.)) else {
    ...
    pub fn encode_tile_set<I>(&mut self, tiles: I)
    where
    ...
    pub fn encode_self_kawa(&mut self, item: Option<&KawaItem>) {
        if let Some(k) = item {
            for kan in k.kan {
    ...
    pub fn encode_kawa(&mut self, item: Option<&KawaItem>) {
        if let Some(k) = item {
